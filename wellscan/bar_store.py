...
class CockroachBarStore:
    ...
    def save_access_snapshot(
        ...
        """Persist one real KIS scanner view for an exact access minute."""

        from psycopg.types.json import Jsonb

        if observed_at.tzinfo is None:
            raise ValueError("접속시각 스냅샷에는 시간대가 필요합니다")
        ...
        try:
            with self._lock, self._connection_session() as connection:
                ...
                with connection.cursor() as cursor:
                    cursor.execute(
                        f"""INSERT INTO {ACCESS_TABLE_NAME} (observed_at, market, session, payload)
                            VALUES (%s, %s, %s, %s)
                            ON CONFLICT (observed_at, market, session) DO UPDATE SET payload=excluded.payload""",
                        (minute, market, session, Jsonb(payload)),
                    )
            self._available, self._last_error = True, ""
            return True
        except Exception as exc:
            self._record_error(exc)
            raise StoreUnavailableError(self._last_error or "접속시각 스냅샷 저장 실패") from exc

    def load_access_snapshots(
        self,
        market: str,
        session: str,
        start: datetime,
        ...
        """Load actual live access observations without filling missing minutes."""
        ...
        try:
            with self._lock, self._connection_session() as connection:
                ...
                with connection.cursor() as cursor:
                    cursor.execute(
                        f"""SELECT observed_at, payload FROM {ACCESS_TABLE_NAME}
                            WHERE market=%s AND session=%s AND observed_at >= %s AND observed_at <= %s
                            ORDER BY observed_at LIMIT %s""",
                        (market, session, start, end, limit),
                    )
                    rows = cursor.fetchall()
            records = []
            for observed_at, payload in rows:
                ...
                if not isinstance(value, dict):
                    raise ValueError("접속시각 스냅샷 payload가 JSON 객체가 아닙니다")
                records.append({"observed_at": observed_at, "market": market, "session": session, **value})
            self._available, self._last_error = True, ""
            return records
        except Exception as exc:
            self._record_error(exc)
            raise StoreUnavailableError(self._last_error or "접속시각 스냅샷 읽기 실패") from exc
